...
def pad_batch_data(insts,
    ...
    # generate input mask according to mask_pos
    # if return_input_mask:
    #     # This is used to avoid attention on paddings.
    #     input_mask_data_1 = np.array([[1] * len(inst) + [0] *
    #                                 (max_len - len(inst)) for inst in insts])
    #     input_mask_data_1 = np.expand_dims(input_mask_data_1, axis=-1)
    #     input_mask_data_2 = np.array([([1] if inst[0] == mask_id else [0]) +
    #                                    [0] + ([1] if inst[2] == mask_id else [0])
    #                                  for inst in insts])
    #     input_mask_data_2 = np.expand_dims(input_mask_data_2, axis=1)
    #     input_mask_data = np.array([np.matmul(input1,input2)
    #       for input1,input2 in zip(input_mask_data_1,input_mask_data_2)])
    #
